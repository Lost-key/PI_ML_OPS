...
def PlayTimeGenre(genre: str):
    ...
    Nota: 
    Esta función utiliza un dataset cargado previamente ('fastAPI_venv/play_time_genre.parquet').
    """
    '''
    ...
    df_max_year_per_genre=pd.read_parquet('fastAPI_venv/play_time_genre.parquet')
...
def UserForGenre(genre: str):
    ...
    Nota:
    Esta función utiliza un dataset cargado previamente ('fastAPI_venv/user_for_genre.parquet').
    """
    ...
    #Se carga el dataset provisto para esta funcion
    df_max_user_per_genre=pd.read_parquet('fastAPI_venv/user_for_genre.parquet')
...
def UsersRecommend(year: int):
    ...
    Nota:
    Esta función utiliza dos datasets cargados previamente ('fastAPI_venv/games_endpoints.parquet' y 'fastAPI_venv/reviews.parquet').
    """
    ...
    try:
        ...
        df_games=pd.read_parquet('fastAPI_venv/games_endpoints.parquet')
        df_reviews_complete=pd.read_parquet('fastAPI_venv/reviews.parquet')
...
def recomendacion_juego(id: int):
    ...
    Nota:
    Esta función utiliza un dataset cargado previamente ('fastAPI_venv/games_recommendations.parquet') que contiene la columna 'recommended_5'.
    """
    #Se verifica que el dato ingresado sea adecuado
    try:
        id = int(id)
        ...
        df_games=pd.read_parquet('fastAPI_venv/games_recommendations.parquet')
